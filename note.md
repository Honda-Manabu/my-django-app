...
Docker Desktop must be in "Engine Running" state while performing any Docker-related operations.
```
...
And yet again, I encountered a series of frustrating twists and turns. I lacked basic knowledge about deployment, and while it was a relief once I finally understood, when I asked about the display and styling of the web page, the generative AI would seriously begin searching for the cause. It was useless no matter what I did (in fact, if I continued asking questions and modifying the code as suggested, skipping the necessary prerequisite work, I would eventually have to abandon everything and start from the beginning with a backup). There was a lot to do when deploying from local to staging. I couldn't make progress because I didn't know what it was. In the process, I finally realized a contradiction: the generative AI doesn't have the insight to know what I should really be doing, unless I ask the right questions.

When you ask the AI ​​about the current phenomenon, it will answer with the most likely of all possibilities. You respond to that answer by saying that it used to work and you haven't changed the settings since, and then you get another answer. That one is also resolved, and you continue to go back and forth with other causes until you finally arrive at the correct answer. When you're dealing with something new and lacking knowledge, this is perhaps unavoidable, but it's also true that you certainly can't solve the problem without the AI.
### Note (25):
#### GitHub Actions Bug (Flaw in IPv6 address support) 
host: ${{ vars.SERVER_HOST }}: In the Actions secrets and variables settings, the hostname in Environment variables was set to the respective domain names for Staging and Production. The deployment-docs.yml entries are exactly the same. Yet, only Staging failed, while Production deployed successfully. Nothing I tried worked, and in the end, the only solution was to change the hostname in Staging's Environment variables to an IP address.
### Note (26):
...
### Note (27):
#### Reworking the task
Even looking at the Amazon SES account dashboard now, the differences aren't immediately clear; however, I had proceeded for about a month and a half using the SMTP method suggested by Gemini, only to ultimately encounter issues connecting the Docker container to the AWS server.
The work involved fixing peripheral configurations—starting with flaws in `deploy-docs.yml` (for automated deployment)—which meant I couldn't focus on the core task of email sending and receiving. The final result was also questionable, so I ultimately decided to scrap that approach.
In fact, I intend to conclude this README by outlining the framework and maintenance principles of the development and operations environment for "MySite" built during this project. When I present this context to Gemini and instruct it to identify the root cause step-by-step—rather than relying on speculation—and to propose solutions only after the cause is determined, it complies obediently at first but eventually reverts to its own style.
So, I decided to give ChatGPT another try. I was amazed by the dramatic progress it had made in just six months. It adhered perfectly to my requirements, and I ended up completely rewriting and correcting the configuration I had previously set up with Gemini. The following sections, up to [7]-E-1, have been revised to reflect this final version.